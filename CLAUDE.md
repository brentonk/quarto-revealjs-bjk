...
- Theme classes are API: `.alert`, `.fg`, `.bg`, `.button`, `.example`, `.implication`, `.tight`, `.invert-colors`, `.wide-table`, `.v-center`, `.etable`, `.columns3070/.columns7030/.columns4060/.columns6040`. Restyle freely; never rename or drop.
...
- **`.implication` list markers are a masked SVG, not a glyph**: no bundled face carries U+21DD (⇝) — upstream Source Sans 3 lacks it entirely, so re-subsetting cannot add it. The arrow is an inline-SVG alpha mask on `li::before`, colored via `background-color` (which is how `.invert-colors` recolors it). Pandoc cannot put a class on an `<li>`, so the class rides on the content span and `.reveal li:has(> .implication)` lifts it to the item; that selector's (0,2,1) also beats the `ul ul li::before` arrow rule's (0,1,3), so implication sub-items get ⇝ rather than →.
